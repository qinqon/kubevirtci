...
func (r *releaser) createGithubRelease(tag, releaseTarballPath string) error {
	...
	release, _, err := client.Repositories.CreateRelease(context.Background(), "qinqon", "kubevirtci", &github.RepositoryRelease{
		TagName: &tag,
		Name:    &tag,
		Body:    &body,
	})
	...
	_, _, err = client.Repositories.UploadReleaseAsset(context.Background(), "qinqon", "kubevirtci", *release.ID, nil, f)
	if err != nil {
